...
export class CargoService extends DependencyService {
	...
	private async processDependencies(
		...
		for await (const [dep, _ver] of Object.entries(dependencies)) {
			const version = typeof _ver == "string" 
				? _ver 
				: typeof _ver == "object"
					? _ver.version
					: null;

			if (!version) throw new Error(`Unable to parse version for crate '${dep}'.`);

			try {
				console.log(`        ${dep}@${version}${isDev ? " (dev)" : ""}`)

				const [extractor] = await this._lookupPackage(dep, version);
				if (!extractor) continue;

				licenses.set(`${dep}@${version}`, (licenses.get(`${dep}@${version}`) || []).concat([]));
			} catch(e) {
				console.error(`Failed to obtain package information for '${dep}@${version}'.`);
				throw e;
			}
		}
	...
	public async process(file: RemoteFile) {
		...
		if (toml["dev-dependencies"]) {
			const devDepsMap = (
				await this.processDependencies(toml["dev-dependencies"], true)
			);
