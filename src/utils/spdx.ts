...
export const getLicenseFileFromSPDX = async (spdx: string) => {
    ...
    const trees = await octokit.request("GET /repos/{owner}/{repo}/git/trees/{tree_sha}", {
        ...common,
        tree_sha: "main"
    });

    const rawTextTree = trees.data.tree.find((tree: any) => tree.path == "text");
    if (!rawTextTree) throw new Error("Unable to locate 'text' tree in spdx/license-list-data.");

    const textTree = await octokit.request("GET /repos/{owner}/{repo}/git/trees/{tree_sha}", {
        ...common,
